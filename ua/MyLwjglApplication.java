...
public class MyLwjglApplication implements Application {
	protected final MyLwjglGraphics graphics;
	protected OpenALAudio audio;
	protected final LwjglFiles files;
	protected final MyLwjglInput input;
	protected final LwjglNet net;
	protected final MyApplicationListener listener;
	...
	public MyLwjglApplication(MyApplicationListener listener, String title, int width, int height) {
		this(listener, createConfig(title, width, height));
	}

	public MyLwjglApplication(MyApplicationListener listener) {
		this(listener, null, 640, 480);
	}

	public MyLwjglApplication(MyApplicationListener listener, MyLwjglApplicationConfiguration config) {
		this(listener, config, new MyLwjglGraphics(config));
	}

	public MyLwjglApplication(MyApplicationListener listener, Canvas canvas) {
		this(listener, new MyLwjglApplicationConfiguration(), new MyLwjglGraphics(canvas));
	}

	public MyLwjglApplication(MyApplicationListener listener, MyLwjglApplicationConfiguration config, Canvas canvas) {
		this(listener, config, new MyLwjglGraphics(canvas, config));
	}

	public MyLwjglApplication(MyApplicationListener listener, MyLwjglApplicationConfiguration config,
			MyLwjglGraphics graphics) {
	...
	private static MyLwjglApplicationConfiguration createConfig(String title, int width, int height) {
		MyLwjglApplicationConfiguration config = new MyLwjglApplicationConfiguration();
		config.title = title;
		config.width = width;
	...
	private void initialize() {
		mainLoopThread = new Thread("LWJGL Application") {
			...
			public void run() {
				...
				try {
					MyLwjglApplication.this.mainLoop();
				} catch (Throwable t) {
					if (audio != null)
						audio.dispose();
	...
	void mainLoop() {
		...
		while (running) {
			...
			if (wasActive && !isActive) { // if it's just recently minimized
											// from active state
				wasActive = false;
			...
			if (!wasActive && isActive) { // if it's just recently focused from
											// minimized state
				wasActive = true;
				synchronized (lifecycleListeners) {
					LifecycleListener[] listeners = lifecycleListeners.begin();
	...
	@Override
	public MyLwjglGraphics getGraphics() {
		return graphics;
	}
