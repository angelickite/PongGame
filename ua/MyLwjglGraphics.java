 ...
 * @author mzechner
 */
public class MyLwjglGraphics implements Graphics {
	...
	MyLwjglApplicationConfiguration config;
	...
	MyLwjglGraphics(MyLwjglApplicationConfiguration config) {
		this.config = config;
	}

	MyLwjglGraphics(Canvas canvas) {
		this.config = new MyLwjglApplicationConfiguration();
		config.width = canvas.getWidth();
		config.height = canvas.getHeight();
		this.canvas = canvas;
	}

	MyLwjglGraphics(Canvas canvas, MyLwjglApplicationConfiguration config) {
		this.config = config;
		this.canvas = canvas;
	}
			...
			if (config.iconPaths.size > 0) {
				...
				for (int i = 0, n = config.iconPaths.size; i < n; i++) {
					Pixmap pixmap = new Pixmap(
							Gdx.files.getFileHandle(config.iconPaths.get(i), config.iconFileTypes.get(i)));
	...
	/**
	 * @return whether the supported OpenGL (not ES) version is compatible with
	 *         OpenGL ES 3.x.
	 */
	private static boolean fullCompatibleWithGLES3() {
		// OpenGL ES 3.0 is compatible with OpenGL 4.3 core, see
		// http://en.wikipedia.org/wiki/OpenGL_ES#OpenGL_ES_3.0
	...
	private static boolean supportsFBO() {
		// FBO is in core since OpenGL 3.0, see
	...
	private void createDisplayPixelFormat(boolean useGL30, int gles30ContextMajor, int gles30ContextMinor) {
		try {
			if (useGL30) {
				ContextAttribs context = new ContextAttribs(gles30ContextMajor, gles30ContextMinor)
						.withForwardCompatible(false).withProfileCore(true);
				try {
					Display.create(new PixelFormat(config.r + config.g + config.b, config.a, config.depth,
	...
	public void initiateGLInstances() {
		if (usingGL30) {
			gl30 = new MyLwjglGL30();
			gl20 = gl30;
		} else {
			gl20 = new MyLwjglGL20();
		}
	...
	public boolean setWindowedMode(int width, int height) {
		...
		try {
			...
			// Workaround for bug in LWJGL whereby resizable state is lost on
			// DisplayMode change
	...
	public DisplayMode getDisplayMode() {
		...
		return new LwjglDisplayMode(mode.getWidth(), mode.getHeight(), mode.getFrequency(), mode.getBitsPerPixel(),
				mode);
	}
	...
	/**
	 * Display must be reconfigured via {@link #setWindowedMode(int, int)} for
	...
	public interface SetDisplayModeCallback {
		/**
		 * If the display creation fails, this method will be called. Suggested
		 * usage is to modify the passed configuration to use a common width and
		 * height, and set fullscreen to false.
		 * 
		 * @return the configuration to be used for a second attempt at creating
		 *         a display. A null value results in NOT attempting to create
		 *         the display a second time
		 */
		public MyLwjglApplicationConfiguration onFailure(MyLwjglApplicationConfiguration initialConfig);
	}
	...
	public void setCursor(com.badlogic.gdx.graphics.Cursor cursor) {
		...
		try {
			Mouse.setNativeCursor(((MyLwjglCursor) cursor).lwjglCursor);
		} catch (LWJGLException e) {
